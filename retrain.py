    ...
    for sub_dir in sub_dirs:
        ...
        for file_name in file_list:
            ...
            if percentage_hash < validation_percentage:
                ...
                val += 1
                print(str(val) + ". validation image: " + file_name)
            elif percentage_hash < (testing_percentage + validation_percentage):
                testing_images.append(base_name)
                tes += 1
                print(str(tes) + ". test image: " + file_name)
            else:
...
def add_input_distortions(flip_left_right, random_crop, random_scale,
  ...
  """
    ...
    resize_scale_value = tf.random_uniform(tensor_shape.TensorShape([]),
                                           minval=1.0,
                                           maxval=resize_scale)
    ...
    brightness_value = tf.random_uniform(tensor_shape.TensorShape([]),
...
def main(_):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        train_writer = tf.summary.FileWriter(FLAGS.summaries_dir + datetime.now().strftime("%Y%m%d-%H%M") + '/train',
                                             sess.graph)

        validation_writer = tf.summary.FileWriter(
            FLAGS.summaries_dir + datetime.now().strftime("%Y%m%d-%H%M") + '/validation')
        ...
        """ img_d_summary_dir = os.path.join(FLAGS.summaries_dir, "summaries", "img")
        img_d_summary_writer = tf.summary.FileWriter(img_d_summary_dir, sess.graph)
        # have to save the testing labels and predicted labels somehow
        img_d_summary = plot_confusion_matrix(y_test,
                                              y_pred,
                                              "training_summaries/retrained_labels.txt",
                                              title="Confusion matrix",
                                              tensor_name='plots/cm')
